...
export const basicInfoSchema = yup.object().shape({
  ...
  applicationEndDate: yup.date()
    ...
    .min(yup.ref('applicationStartDate'), '応募終了日は開始日より後にしてください'),

  contestStartDate: yup.date().required('開催開始日を選択してください'),
  contestEndDate: yup.date()
...
export const prizeSchema = yup.object().shape({
  ...
  prizeDistribution: yup.array()
    .min(1, '報酬配分を設定してください')
    .test('total-matches-pool', '配分金額の合計が総賞金額と一致しません', function(value) {
...
export type ContestCreateFormData = yup.InferType<typeof basicInfoSchema> & yup.InferType<typeof briefSchema> & yup.InferType<typeof resourcesSchema> & yup.InferType<typeof prizeSchema>;