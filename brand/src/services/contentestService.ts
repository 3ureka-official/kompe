...
export const createContest = async (
  ...
  if (thumbnailFile) {
    ...
    const { data: thumbData, error: thumbErr } =
      await supabase.functions.invoke("get_upload_url", {
  ...
  if (assetsData) {
    for (const asset of assetsData) {
      if (asset.file) {
        ...
        const { data: assetData, error: assetErr } =
          await supabase.functions.invoke("get_upload_url", {
        ...
        if (!assetSignedUrl)
          throw new Error("Failed to get signedUrl for asset");
  ...
  const { data, error } = await supabase.functions.invoke("create_contest", {
    body: JSON.stringify({
      brandId,
      contestData,
      thumbnailKey,
