...
import {
  ContestFilters as ContestFiltersType,
  ContestSortBy,
  CONTEST_CATEGORIES,
  CONTEST_STATUS,
  ContestCategory,
...
export function ContestFilters({
  ...
  const currentSortLabel =
    SORT_OPTIONS.find((option) => option.value === sortBy)?.label || "新着順";
            ...
            {hasActiveFilters && (
              <Badge variant="destructive" className="ml-1 px-1 py-0 text-xs">
                {(filters.categories?.length || 0) +
                  ...
                  (filters.minPrizeAmount ? 1 : 0) +
                  (filters.maxPrizeAmount ? 1 : 0) +
                  (search ? 1 : 0)}
              </Badge>
            )}
          </Button>
        ...
        <div className="flex flex-wrap gap-2">
          {filters.categories?.map((category) => (
            ...
            >
              ...
              <button
                onClick={() =>
                  handleCategoryToggle(category as ContestCategory)
                }
