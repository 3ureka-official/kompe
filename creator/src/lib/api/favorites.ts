...
export async function getUserFavorites(
  ...
  if (search) {
    ...
    favoriteContests = favoriteContests.filter(
      (contest) =>
  ...
  favoriteContests.sort((a, b) => {
    ...
    return (
      new Date(favB.favoriteAt).getTime() - new Date(favA.favoriteAt).getTime()
    );
  });
...
export async function getUserFavorite(
  ...
  const favorite = favorites.find(
    (fav) => fav.userId === userId && fav.contestId === contestId,
  );
