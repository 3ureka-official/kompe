import { Contest, ContestSummary, ContestFilters, ContestSortBy, ContestListResponse, ContestCategory, ContestStatus, EvaluationType } from '@/types/contest'
...
function buildContestDetail(contestId: string): Contest | null {
  ...
  // 応募情報を取得
  const contestSubmissions = submissions.filter(s => s.contest_id === contestId && s.submission_status === 'approved')
