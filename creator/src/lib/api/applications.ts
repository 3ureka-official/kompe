...
export async function getUserApplications(
  ...
  if (search) {
    ...
    appliedContests = appliedContests.filter(
      (contest) =>
  ...
  appliedContests.sort((a, b) => {
    ...
    return (
      new Date(appB.appliedAt).getTime() - new Date(appA.appliedAt).getTime()
    );
  });
...
export async function getUserApplication(
  ...
  const application = applications.find(
    (app) => app.userId === userId && app.contestId === contestId,
  );
